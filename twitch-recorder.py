...
class TwitchRecorder:
    def __init__(self):
        ...
        self.client_id = "jzkbprff40iqj646a697cyrvl0zt2m6" # не изменять
        self.oauth_token = "xxxxx"
        self.ffmpeg_path = 'ffmpeg'
        self.refresh = 15.0
        self.root_path = "D:\canal\create"
        
        
        self.username = ""   #никнейм канала
        self.quality = ""   #качество трансляции
    ...
    def run(self):
        ...
        if(self.refresh < 15):
            print(datetime.datetime.now().strftime("%H:%M:%S"),"Check interval should not be lower than 15 seconds.")
            self.refresh = 15
        ...
        try:
            ...
            if(len(video_list) > 0):
                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),'Fixing previously recorded files.')
            for f in video_list:
                recorded_filename = os.path.join(self.recorded_path, f)
                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),'Fixing ' + recorded_filename + '.')
                try:
        ...
        print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),"Checking for", self.username, "every", self.refresh, "seconds. Record with", self.quality, "quality.")
        self.loopcheck()
    ...
    def loopcheck(self):
        while True:
            ...
            if status == 2:
                ...
                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),self.username, "currently offline, checking again in", self.refresh, "seconds.")
                time.sleep(self.refresh)
            elif status == 0:
                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),self.username, "online. Stream recording in session.")
                filename = self.username + " - " + datetime.datetime.now().strftime("%Y-%m-%d %Hh%Mm%Ss") + ".mpeg"
                ...
                subprocess.call(
                    ["livestreamer", "--output", recorded_filename, "https://www.twitch.tv/" + self.username, self.quality])


                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),"Recording stream is done. Fixing video file.")
                if(os.path.exists(recorded_filename) is True):
                    ...
                    print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),"Skip fixing. File not found.")
                    
                print(datetime.datetime.now().strftime("%Hh%Mm%Ss"),"Fixing is done. Going back to checking..")
                time.sleep(self.refresh)
